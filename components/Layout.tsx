...
interface LayoutProps {
  ...
  user: User;
  currentView: string;
  activeModule: 'trip' | 'trip_invoice' | 'inspection';
  lang: Language;
  onLogout: () => void;
  onLangChange: (lang: Language) => void;
  onNavigate: (view: 'dashboard' | 'create' | 'switch_module' | 'trips' | 'trip_invoices' | 'inspections') => void;
}
...
export const Layout: React.FC<LayoutProps> = ({ children, user, currentView, activeModule, lang, onLogout, onLangChange, onNavigate }) => {
  ...
  const moduleLabel = activeModule === 'trip_invoice' ? t.tripInvoices : activeModule === 'inspection' ? t.inspections : t.trips;
      ...
      <header className="sticky top-0 z-50 app-header px-4 pt-4 pb-3 shadow-xl shadow-black/20">
        <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-[9px] font-black text-white/40 uppercase tracking-[0.22em] leading-none">{moduleLabel}</p>
              <h1 className="font-black text-base truncate tracking-tight mt-1">{t.appName}</h1>
            </div>
            <button
